...
public class LineGraphManager : MonoBehaviour
{
    ...
    public List<TextMesh> xPointMarkers;
    ...
    // Comment the start function and directly call setGraph
    void Start()
    {
        ...
        // showing graph
        setGraph("Sample Graph","Test1", data1, "Test2", data2);
    }
    public void setGraph(String title,String name1, List<float> data1,
    String name2, List<float> data2)
	{
        graphDataPlayer1.Clear();
        graphDataPlayer2.Clear();
        int minLen = data1.Count < data2.Count ? data1.Count : data2.Count;
        highestValue = data1[0];
        for (int i = 0; i < minLen;i++)
		{
            GraphData gd = new GraphData();
            gd.marbles = data1[i];
			if(data1[i]>highestValue)
			{
                highestValue = data1[i];
            }
            graphDataPlayer1.Add(gd);
            GraphData gd2 = new GraphData();
            gd2.marbles = data2[i];
            if (data2[i] > highestValue)
            {
                highestValue = data2[i];
            }
        ...
        for (int i = 0; i < xPointMarkers.Count;i++)
		{
            xPointMarkers[i].text = ((int)((highestValue / xPointMarkers.Count) * (i + 1))).ToString();
        }
